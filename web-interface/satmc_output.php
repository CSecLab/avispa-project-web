...
// Constructing Command line arguments for tool execution
$args=" ";
$args .= "--solver=".$_POST['solver']." ";
$args .= "--enc=gp-efa ";
$args .= "--max=".$_POST['depth']." ";
if ($_POST['mutex'])  $args .= "--mutex=0 ";
         else $args .= "--mutex=1 ";
if ($_POST['ct'])      $args .= "--ct=true ";
         else $args .= "--ct=false ";
if ($_POST['oi'])      $args .= "--oi=true ";
         else $args .= "--oi=false ";
...
if($avispawebsetting == remote){
  ...
  $satmc_cmd = "(ssh".$ssh_options.$remoteuser."@".$remoteip." ".
                      ...
                      $localwebinterfacepath.$_POST['opened_file'].".satmc.atk";

}
elseif ($avispawebsetting == local)
{
  $hlpsl2if_cmd = $bindir.$hlpsl2if." --all ".$_POST['opened_file']." 2>&1";
  $satmc_cmd = $bindir.$satmc." ".$localwebinterfacepath.$_POST['opened_file'].".if".$args." > ".$localwebinterfacepath.$_POST['opened_file'].".satmc.atk";
} 
else 
  echo "AVISPA_WEB_SETTING variable not properly set in config.ini<br>";
...
function Summary($atk_file)
{
  ...
  // Search for the ".satmc.atk"SUMMARY keyword inside the file
  $found = false;
...
<td valign="top">
    ...
    <form name="forminput" method="post" action="satmc_input.php">

      <input type="hidden" name="opened_file" value="<?php echo $_POST['opened_file']?>">
      <input type="hidden" name="changing" value=1>
      <input type="hidden" name="solver" value="<?php echo $_POST['solver']?>">
      <input type="hidden" name="enc" value="<?php echo $_POST['enc']?>">
      <input type="hidden" name="depth" value="<?php echo $_POST['depth']?>">
      <input type="hidden" name="mutex" value=<?php echo $_POST['mutex']?>>
      <input type="hidden" name="ct" value=<?php echo $_POST['ct']?>>
      <input type="hidden" name="oi" value=<?php echo $_POST['oi']?>>
	...
	<br><br>
        ...
        <textarea name="textfieldinput" rows="<?php echo $textarea_rows?>" cols="<?php echo $textarea_cols?>" wrap="off" readonly
	><?php
          ...
          else
	  {
          ...
          if ($translation_ok) 
          {
            ...
            // Show Attack file
            if (!readfile($_POST['opened_file'].".satmc.atk"))
              echo "File ".$_POST['opened_file'].".satmc.atk"." cannot be opened";
	...
	<!-- TextArea Buttons -->
	<a href=# onclick="window.open('showfile.php?file=<?php echo $_POST['opened_file'] 
?>&tool=satmc','view_hlpsl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
	...
	<?php
	  ...
	  {
	    // IF Button
	    echo "<a href=# onclick=\"window.open('showfile.php?file=".$_POST['opened_file'].".if&tool=satmc','view_if','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=0');return(false)\"><img src=\"images/view_if.gif\" border=\"0\"></a>\n"; 
	    
	    // MSC and PS Buttons
	    $satmc_summary = Summary($_POST['opened_file'].".satmc.atk");
	    if ($satmc_summary=="UNSAFE")
	    {
	      echo "<a href=# onclick=\"window.open('showfile.php?image=".$_POST['opened_file'].".satmc.atk&tool=satmc','satmc_msc','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_msc.gif\" border=\"0\"></a>\n";
	      
	      echo "<a href=# onclick=\"window.open('showfile.php?ps=".$_POST['opened_file'].".satmc.atk&tool=satmc','satmc_ps','width=500,height=300,left=262,top=234,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_ps.gif\" border=\"0\"></a>\n";
	...
	<?php 
	  if ($translation_ok)
	    echo "<img src=\"images/trace_satmc.gif\" border=\"0\" usemap=\"#Map\">\n";
	  else
	    echo "<img src=\"images/trace_hlpsl2if_error.gif\" border=\"0\">\n";
	?>
	<map name="Map">
          <area shape="rect" coords="7,141,71,164" href="ofmc_input.php?opened_file=<?php echo $_POST['opened_file'] ?>">
          <area shape="rect" coords="79,141,145,162" href="atse_input.php?opened_file=<?php echo $_POST['opened_file'] ?>">
          <area shape="rect" coords="229,141,293,162" href="ta4sp_input.php?opened_file=<?php echo $_POST['opened_file'] ?>">
        </map>
	<br>
        </td>
