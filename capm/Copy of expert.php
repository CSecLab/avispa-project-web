...
{
  $handle = fopen ($_POST['opened_file'],"w");
  ...
  If (!fwrite($handle, stripcslashes($textinputarea), $file_length))
  {
    ...
    echo "Cannot write to file ".$_POST['opened_file'];
  }
  fclose($handle);
}
...
<td valign="top">
  ...
  <?
    ...
    if (   ($_FILES['user_file'] && ($_POST['load_user_x'] != "")) // user_file has a value and Load File was pressed 
  ...
  ><?
    ...
    //echo "load_user=".$_POST['load_user_x']."\n";
    //echo "load_test=".$_POST['load_test_x']."\n";
    //echo "opened_file=".$_POST['opened_file']."\n";
    //echo "save_file=".$_POST['save_file_x']."\n";
    //echo "edit_file=".$_POST['edit_file_x']."\n";
    ...
    if ($_FILES['user_file'] && ($_POST['load_user_x'] != ""))
    {
      $_POST['opened_file'] = tempnam($tempdir, "workfile");
      $_POST['opened_file'] = $tempdir.basename($_POST['opened_file']);

      // Try to move the file to the working file
      // If the move operation fail, print some debugging info
      if (move_uploaded_file($_FILES['user_file']['tmp_name'], $_POST['opened_file']))
      {
        // Show the file in the textarea
        if (!readfile($_POST['opened_file']))
          echo "File ".$_POST['opened_file']." was sucessfuly uploaded but it cannot be opened";
      } else {   
    ...
    else 
    {
       ...
       if ($_POST['test_file'] && ($_POST['load_test_x'] != ""))
       {
         ...
         if (!copy($predefpath.$_POST['test_file'], $_POST['opened_file']))
         {
            echo "Test File copying operation failed";
            unlink($_POST['opened_file']);
            $_POST['opened_file'] = "";
         }
       } 
       else 
       {
         ...
         {
           if (!readfile($_POST['opened_file']))
             echo "File ".$_POST['opened_file']." cannot be opened";
         } 
         else 
         {
  ...
  <?php
    ...
    if ($_POST['edit_file_x'] != "")
    {
      echo "<input type=\"hidden\" name=\"opened_file\" value=\"".$_POST['opened_file']."\">";
  ...
  <?
    if ($_POST['opened_file'] && ($_POST['edit_file_x'] == ""))
    {
      echo "<area shape=\"rect\" coords=\"113,46,187,67\" href=\"hlpsl2if_input.php?opened_file=".$_POST['opened_file']."\">";
      echo "<area shape=\"rect\" coords=\"7,141,71,164\" href=\"ofmc_input.php?opened_file=".$_POST['opened_file']."\">";
      echo "<area shape=\"rect\" coords=\"79,141,145,162\" href=\"atse_input.php?opened_file=".$_POST['opened_file']."\">";
      echo "<area shape=\"rect\" coords=\"155,141,219,162\" href=\"satmc_input.php?opened_file=".$_POST['opened_file']."\">";
      echo "<area shape=\"rect\" coords=\"229,141,293,162\" href=\"ta4sp_input.php?opened_file=".$_POST['opened_file']."\">";
    }
  ?>
  </map>
  <br>
