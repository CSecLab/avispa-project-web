...
$args=" ";
if ($sessco) $args = $args."-sessco ";
if ($depth)  $args = $args."-d ".$depth." ";
if ($path)   $args = $args."-p ".$path. " ";
...
elseif ($avispawebsetting == local)
{
  ...
  $ofmc_cmd = $bindir.$ofmc." ".$localwebinterfacepath.$opened_file.".if".$args."> ".$localwebinterfacepath.$opened_file.".ofmc.atk";
} 
else 
...
<td valign="top">
	...
	><?php
	  ...
	  // If the translation was sucessuful show the file in the textarea,
          ...
          if ($translation_ok) 
          {
            ...
            // Show Attack file
	    if (!readfile($opened_file.".ofmc.atk"))
              echo "File ".$opened_file.".ofmc.atk"." cannot be opened";
	...
	<!-- TextArea Buttons -->
	<a href=# onclick="window.open('showfile.php?file=<? echo $opened_file ?>&tool=ofmc','view_hlpsl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
	...
	<?php
	  ...
	  {
	    // IF Button
	    echo "<a href=# onclick=\"window.open('showfile.php?file=".$opened_file.".if&tool=ofmc','view_if','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=0');return(false)\"><img src=\"images/view_if.gif\" border=\"0\"></a>\n"; 
