...
elseif ($avispawebsetting == local)
{
  ...
  $ta4sp_cmd = $bindir.$ta4sp." ".
               $localwebinterfacepath.$opened_file.".if ".$args." > ".
               $localwebinterfacepath.$opened_file.".ta4sp.atk";
} 
else 
...
<td valign="top">
	  ...
	  // Call the tools on the workfile
	    ...
	    $hlpsl2if_output = shell_exec($hlpsl2if_cmd);
          ...
          if ($translation_ok) 
          {
            ...
            // Show Attack file
	    if (!readfile($opened_file.".ta4sp.atk"))
              echo "File ".$opened_file.".ta4sp.atk"." cannot be opened";
	...
	<!-- TextArea Buttons -->
	<a href=# onclick="window.open('showfile.php?file=<? echo $opened_file ?>&tool=ta4sp','view_hlpsl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
	...
	<?php
	  ...
	  {
	    // IF Button
	    echo "<a href=# onclick=\"window.open('showfile.php?file=".$opened_file.".if&tool=ta4sp','view_if','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=0');return(false)\"><img src=\"images/view_if.gif\" border=\"0\"></a>\n"; 
