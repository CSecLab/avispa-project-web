...
if ($_POST['v'])      
  ...
  $args .= "-col 10000 ";
if ($_POST['notype']) $args .= "-notype ";
$args .= "-".$_POST['algorithm']." ";
...
if ($avispawebsetting == remote){
  ...
  $atse_cmd = "(ssh".$ssh_options.$remoteuser."@".$remoteip." ".
                     $remotewebinterfacepath.$bindir.$atse.$args.$remotewebinterfacepath.$_POST['opened_file'].".if) > ".
                     $localwebinterfacepath.$_POST['opened_file'].".atse.atk";

}
elseif ($avispawebsetting == local)
{
  $hlpsl2if_cmd = $bindir.$hlpsl2if." --all ".$_POST['opened_file']." 2>&1";
  $atse_cmd = $bindir.$atse.$args.$localwebinterfacepath.$_POST['opened_file'].".if > ".$localwebinterfacepath.$_POST['opened_file'].".atse.atk";
} 
else 
...
<td valign="top">
    ...
    <form name="forminput" method="post" action="atse_input.php">
      ...
      <input type="hidden" name="notype" value=<?php echo $_POST['notype']?>>
      <input type="hidden" name="td" value=<?php echo $_POST['td']?>>
      <input type="hidden" name="lr" value=<?php echo $_POST['lr']?>>
	...
	><?php
          ...
          else
	  {
          ...
          if ($translation_ok) 
          {
            ...
            // Execute ATSE
            if ($debug) echo "ATSE: ".$atse_cmd."\n";
            exec($atse_cmd);

            // Show Attack file
	    if (!readfile($_POST['opened_file'].".atse.atk"))
              echo "File ".$_POST['opened_file'].".atse.atk"." cannot be opened";
...
?>&tool=atse','view_hlpsl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
	...
	<?php
	  ...
	  {
	    // IF Button
	    echo "<a href=# onclick=\"window.open('showfile.php?file=".$_POST['opened_file'].".if&tool=atse','view_if','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=0');return(false)\"><img src=\"images/view_if.gif\" border=\"0\"></a>\n"; 
	    
	    // MSC and PS Buttons
	    $atse_summary = Summary($_POST['opened_file'].".atse.atk");
	    if ($atse_summary=="UNSAFE")
	    {
	      echo "<a href=# onclick=\"window.open('showfile.php?image=".$_POST['opened_file'].".atse.atk&tool=atse','atse_msc','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_msc.gif\" border=\"0\"></a>\n";
	      
	      echo "<a href=# onclick=\"window.open('showfile.php?ps=".$_POST['opened_file'].".atse.atk&tool=atse','atse_ps','width=500,height=300,left=262,top=234,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_ps.gif\" border=\"0\"></a>\n";
	...
	<?php 
	  if ($translation_ok)
	    echo "<img src=\"images/trace_atse.gif\" border=\"0\" usemap=\"#Map\">\n";
	  else
	...
	<map name="Map">
          ...
          <area shape="rect" coords="155,141,219,162" href="satmc_input.php?opened_file=<?php echo $_POST['opened_file'] ?>">
