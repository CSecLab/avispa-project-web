...
if($avispawebsetting == remote){
  ...
  $satmc_cmd = "(ssh".$ssh_options.$remoteuser."@".$remoteip." ".
                      ...
                      $remotewebinterfacepath.$opened_file.".if ".$args.") > ".
...
elseif ($avispawebsetting == local)
{
  ...
  $satmc_cmd = $bindir.$satmc." ".$localwebinterfacepath.$opened_file.".if".$args." > ".$localwebinterfacepath.$opened_file.".satmc.atk";
} 
else 
  echo "AVISPA_WEB_SETTING variable not properly set in config.ini<br>";
...
<td valign="top">
    ...
    <form name="forminput" method="post" action="satmc_input.php">
      ...
      <input type="hidden" name="mutex" value=<?=$mutex?>>
	...
	><?php
          ...
          else
	  {
          ...
          if ($translation_ok) 
          {
            ...
            // Show Attack file
            if (!readfile($opened_file.".satmc.atk"))
              echo "File ".$opened_file.".satmc.atk"." cannot be opened";
	...
	<!-- TextArea Buttons -->
	<a href=# onclick="window.open('showfile.php?file=<? echo $opened_file ?>&tool=satmc','view_hlpsl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
	...
	<?php
	  ...
	  {
	    // IF Button
	    echo "<a href=# onclick=\"window.open('showfile.php?file=".$opened_file.".if&tool=satmc','view_if','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=0');return(false)\"><img src=\"images/view_if.gif\" border=\"0\"></a>\n"; 
	    
	    // MSC and PS Buttons
	    $satmc_summary = Summary($opened_file.".satmc.atk");
	    if ($satmc_summary=="UNSAFE")
	    {
	      echo "<a href=# onclick=\"window.open('showfile.php?image=".$opened_file.".satmc.atk&tool=satmc','satmc_msc','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_msc.gif\" border=\"0\"></a>\n";
	      
	      echo "<a href=# onclick=\"window.open('showfile.php?ps=".$opened_file.".satmc.atk&tool=satmc','satmc_ps','width=500,height=300,left=262,top=234,menubar=0,status=0,location=0,toolbar=0,scrollbars=1,resizable=0');return(false)\"><img src=\"images/view_ps.gif\" border=\"0\"></a>\n";
