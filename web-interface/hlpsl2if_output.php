...
$args=" ";
if ($_POST['types']) $args = $args."--types ";
if ($_POST['inits']) $args = $args."--init ";
if ($_POST['rules']) $args = $args."--rules ";
if ($_POST['goals']) $args = $args."--goals ";
...
if ($avispawebsetting == remote)
{
  $copy_hlpsl_remote = "scp -p".$ssh_options.$localwebinterfacepath.$_POST['opened_file']." ".
                             $remoteuser."@".$remoteip.":".
                             $remotewebinterfacepath.$_POST['opened_file'];
...
{
  $hlpsl2if_cmd = $bindir.$hlpsl2if." ".$args.$_POST['opened_file']." 2>&1";
}
else 
...
<td valign="top">
  ...
  <form name="forminput" method="post" action="hlpsl2if_input.php">
    ...
    <input type="hidden" name="types" value=<?php echo $_POST['types']?>>
    <input type="hidden" name="inits" value=<?php echo $_POST['inits']?>>
    <input type="hidden" name="rules" value=<?php echo $_POST['rules']?>>
	...
	else
	{
	  ...
	  $hlpsl2if_output = shell_exec($hlpsl2if_cmd);
        ...
        if ($translation_ok) 
        {
          //readfile ($_POST['opened_file'].".if");
          
          // Show IF file
	  if (!readfile($_POST['opened_file'].".if"))
            echo "File ".$_POST['opened_file'].".if cannot be opened";
...
?>','view_hlspl','width=1000,height=600,left=12,top=84,menubar=0,status=0,location=0,toolbar=0,scrollbars=0,resizable=1');return(false)"><img src="images/view_hlpsl.gif" border="0"></a>
      <br>      
      </td>
    </tr>
